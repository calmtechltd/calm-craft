...
description: Assess a bug report against authoritative intent and observed behavior, including an insufficient-evidence outcome. Use when the user asks to classify, diagnose, or report on a suspected defect, or as the diagnostic stage of an authorized fix. A mention of broken behavior inside an implementation request does not trigger this skill. This skill reports; the regression workflow handles an authorized confirmed fix.
---
...
If the original request authorizes a fix and triage confirms the bug, continue into `bug-regression-red-green` in the same turn without asking the user to repeat that authorization. During active implementation, a concrete, in-scope suggestion requests implementation when the intended result is clear. Pause for unresolved product intent or insufficient evidence that blocks a safe fix, a destructive or external action that needs authorization, or a material expansion of scope.
...
Report the verdict, governing evidence, actual versus expected outcome, confidence/limits, and next action. For a confirmed runtime regression, use `bug-regression-red-green` for an authorized fix after applying test-value policy. For uncertain intent, ask only the necessary question and leave independent authorized work free to continue.
